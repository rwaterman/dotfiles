...
- Push feature branches without asking, including the first push (`git push -u origin <branch>`). Do not push directly to a repo's default branch or to `develop`, and do not force-push a branch someone else may have pulled — those need an explicit ask.
